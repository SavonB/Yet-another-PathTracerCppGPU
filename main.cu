...
__global__ void create_world(hitable** d_list, hitable** d_world,
    camera** d_camera, int aspect,vec3 lookfrom, vec3 lookat, vec3 vup, float fvov, float aperture) {
    if (threadIdx.x == 0 && blockIdx.x == 0) {
        ...
        *d_camera = new camera(fvov,aspect,lookfrom,lookat,vup,aperture,focus_dist);
    }
}
...
void run(int nx, int ny, int ns, vec3 lookfrom, vec3 lookat, vec3 vup, float fvov_degrees, float aperture,std::string filename) {
    ...
    create_world << <1, 1 >> > (d_list, d_world, d_camera, int(nx / ny), lookfrom, lookat, vup, fvov_degrees, aperture);
